...
class Database():
    ...
    def create(self):
        ...
        cursor.execute("CREATE TABLE users(user_name VARCHAR(16) NOT NULL UNIQUE PRIMARY KEY, public_key VARCHAR(392) NOT NULL UNIQUE, key_file VARCHAR(1764) COLLATE ascii_general_ci NOT NULL UNIQUE, time_created INT NOT NULL, profile_picture VARCHAR(64) NOT NULL, info VARCHAR(255));")
        cursor.execute("CREATE TABLE posts(id VARCHAR(23) NOT NULL PRIMARY KEY, user_id VARCHAR(16) NOT NULL, post VARCHAR(255) NOT NULL, flags VARCHAR(10) NOT NULL, time_posted INT NOT NULL, signature VARCHAR(344), FOREIGN KEY (user_id) REFERENCES users (user_name));")
        cursor.execute("CREATE TABLE comments(id INT NOT NULL PRIMARY KEY, user_id VARCHAR(16) NOT NULL, post_id VARCHAR(23) NOT NULL, comment VARCHAR(255) NOT NULL, FOREIGN KEY (user_id) REFERENCES users (user_name), signature VARCHAR(344)c, FOREIGN KEY (post_id) REFERENCES posts (id));")
