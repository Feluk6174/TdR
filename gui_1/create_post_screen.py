...
class PostUserScreen (Screen):
    def __init__(self, conn, **kwargs):
        ...
        self.sources_post_input = TextInput(multiline = True, size_hint = (1, 0.5), text = "Sources")
        self.content_grid.add_widget(self.sources_post_input)
    ...
    def send_post_press(self, instance):
        ...
        self.send_post_final(self.flag_list, str(self.main_post_content_input.text), str(self.sources_post_input.text))
        self.main_post_content_input.text = ""
        self.sources_post_input.text = ""
        for y in range (len(self.all_flags) - 1):
            if self.all_flags[y + 1][3] == 1:
                self.all_flags[y + 1][2].trigger_action(duration = 0)

    def send_post_final(self, post_flags, text_content, text_sources):
        ...
        #post_likes = nlikes
        #date = int(time.time())
        post_id = hash(str(text_content) + str(text_sources) + str(user_name) + str(post_flags))
        if post_id < 0:
            post_id = post_id * (-1)
        conn.send_post(text_content, text_sources, post_id, post_flags, user_name, private_key)    
